...
#
gee1step.gaussian <- function(dx, formula, X_, Y_, namesd, N_clusters, weight, ...) {
  ...
  ###
  if(is.null(weight))   weight <- rep(1, nrow(dx))
  ...
  ### Estimate beta
  wi <- lapply(1:N_clusters, function(i) .getW(dx[cname_ == i], namesd, rho, dx[cname_ == i, weight]))
  W <- Reduce("+", wi)
  
  di <- lapply(1:N_clusters, function(i) .getD(dx[cname_ == i], namesd, rho, dx[cname_ == i, weight]))
  D <- Reduce("+", di)
  ...
  wi <- lapply(1:N_clusters, function(i) .getW(dr[cname_ == i], namesd, rho, dx[cname_ == i, weight]))
  W <- Reduce("+", wi)
  
  ui <- lapply(1:N_clusters, function(i) .getU(dr[cname_ == i], namesd, rho, dx[cname_ == i, weight]))
  U <- Reduce("+", ui)
