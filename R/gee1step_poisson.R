# Estimate parameters using one-step algorithm (with Poisson distribution)
...
#
gee1step.poisson <- function(dx, formula, X_, Y_, namesd, N_clusters, orig_call, ...) {
  ...
  glmfit <- stats::glm(newform, data = dx, family = stats::poisson) # specific to dist

  dx[, p := stats::predict.glm(glmfit, type = "response")]
  dx[, v := p] # specific to dist
  dx[, resid := (Y - p) / sqrt(v) ]

  dX <- dx[, X_, with = FALSE]
  dX <- dX * dx[, p] # specific to dist
  setnames(dX, namesd)
  dx <- cbind(dx, dX)
  ...
  dr[, lrate := dvars %*% beta2] # specific to dist
  dr[, p := exp(lrate)] # specific to dist
  dr[, v := p] #  # specific to dist
  dr[, resid := ( Y - p) / sqrt( v )]
  dr[, residv := ( Y - p) / v ]

  dR <- dr[, X_, with = FALSE]
  dR <- dR * dr[, p] # specific to dist
  setnames(dR, namesd)
  dr <- cbind(dr, dR)
